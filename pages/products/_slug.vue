    ...
    <div class="container mt-8">
      <!--  -->
        ...
        <div class="flex flex-wrap" v-if="item">
            ...
            <div class="w-full md:w-1/2">
              <div class="flex justify-between items-start" >
                <!--  -->
                <div class="flex flex-col overflow-auto mr-2 h-96">
                  <div class="">
                      <si-image width="100" class="bg-white cursor-pointer mb-2" v-for="(image, index) in item.images" @click="setImage(index)" :key="index" :src="image.src" :alt="`${item.name} - ${image.title}`"/>
                  </div>
                </div>
                <!--  -->
                <div class="relative flex flex-col">
                    <!--  -->
                    <si-image width="450" class="cursor-pointer bg-white"  @click="$store.state.fullImage=image ? image.src : null" :src="image ? image.src : null " :alt="item.name"/>
                </div>
                <!--  -->
              </div>
            </div>
            ...
            <div class="w-full md:w-1/2">
                <div class="p-2">
                    <div class="bg-white p-2">
                        ...
                        <div class="flex justify-between items-center">
                          ...
                          <div>
                            ...
                            <si-product-price class="flex " :type="'simple'" :price="price" :variants="[]"></si-product-price>
                          </div>
                        ...
                        <!--  -->
                        <p class="text-sm text-gray-400 mt-3">{{ item.description }}</p>
                        ...
                        <div class="flex flex-wrap md:flex-nowrap mt-8">
                          ...
                          <!--  -->
                            <button v-if="$settings.sections.product.add_to_cart.active" @click="addToCart" class="btn_add rounded-3xl h-10 px-5 mt-1.5 text-white click-effect mr-5">
                                <span>{{ $settings.sections.product.add_to_cart.text }}</span>
                            </button>
                            <!--  -->
                            <div>
                              <button v-if="$store.state.wishlist.find(i=>i._id==item._id)" @click="removeFromWishlist" title="Wishlist" class="item text-black m-2 p-2 border border-black rounded-full mx-1">
                              ...
                              <button v-else @click="addToWishlist" title="Wishlist" class="item  m-2 p-2 border border-black rounded-full text-black mx-1">
                    ...
                    <!--  -->
                    <!-- <button v-if="$settings.sections.product.buy_now.active" v-show="!$store.state.apps.find(a=>a.placement.indexOf('REPLACE_BUYNOW') >= 0)" @click="buyNow" class="w-full flex ai-c p-2 justify-center bg-blue-400 text-white click-effect">
        ...
        <div class="flex flex-col mt-20">
            ...
            <div  v-if="item" class="bg-white rounded-md p-2 my-3 mx-2 description text-sm" id="description" v-html="item.html"></div>
