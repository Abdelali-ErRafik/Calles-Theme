<template>
    <div class="flex items-center justify-center">
        <button @click="inc(-1)" class="border-b border-t border-l border-black rounded-bl-full rounded-tl-full py-2 px-4" :class="value > quantity.min ? 'bg-white text-black text-bold' : ''">-</button>
        <div class="flex">
            ...
            <span class="flex items-center bg-black px-2 text-bold" v-if="quantity.unit">{{ quantity.unit }}</span>
        </div>
        <button @click="inc(1)" class="border-b border-t border-r border-black rounded-br-full rounded-tr-full py-2 px-4 " :class="value < quantity.instock ? 'bg-white text-black text-bold' : ''">+</button>
    </div>
</template>
