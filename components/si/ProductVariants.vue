<template>
    <div class="options">
        <div v-for="(option, i) in options" :key="i" :class="option.key" class="bg-gray-50 border">
            <b class="capitalize option-name flex">{{ option.name }}</b>
        ...
        <si-product-price v-if="showPrice" class="flex" :type="'simple'" :price="selected.price" :variants="[]"></si-product-price>
    </div>
</template>
<script>
export default {
