<template>
    <div>
        <div class="flex justify-between flex-wrap relative mb-2 p-2 ">
          <hr class="my-2" />
          <!--  -->
            <div class="flex md:w-1/2">
                <div class="flex items-center w-full">
                    <nuxt-link class="w-1/4 flex" :to="`/products/${item.slug}`">
                        <si-image class="rounded-md object-contain w-full" :src="item.image" :alt="item.name"/>
                    </nuxt-link>
                    <div class="w-3/4">
                        <div class="flex flex-col p-2">
                            <nuxt-link class="text-xl" :to="`/products/${item.slug}`">{{ item.name }}</nuxt-link>
            ...
            <div class="flex flex-col justify-center w-1/2 md:w-1/4 bg-gray-100 md:bg-white p-2 relative">
                <div class="text-gray-600">
                    <b class="text-sm">{{ item.quantity.value }} x</b>
                    <b class="text-sm">{{ item.price }} {{ $store.state.currency.symbol }}</b>
                </div>
                <h2 class="text-sm text-red-700">{{ item.total }} {{ $store.state.currency.symbol }}</h2>
            </div>
            <!--  -->
