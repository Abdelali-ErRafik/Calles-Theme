<template>
    <div>
        ...
        <!--  -->
        <div class="price flex justify-start" v-if="type=='simple'">
          <!--  -->
            <span class="relative text-xs" v-if="price.comparePrice > 0">
                <span class="text-red-500">{{ price.comparePrice }}{{ $store.state.currency.symbol }}</span>
                <i class="compare-price absolute top-2 block w-full bg-red-600"></i>
            </span>
            <meta itemprop="price" :content="price.salePrice" />
            <!--  -->
            <span class="flex w-2"></span>
            <!--  -->
            <span class="text-back text-xs">{{ price.salePrice }}{{ $store.state.currency.symbol }}</span>
            <!--  -->
        </div>
        <!--  -->
